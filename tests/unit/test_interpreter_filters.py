    ...
    class TestCommandExecution:
        def test_executes_command_connected_to_include_filter(
            ...
            assert_that(cmds_implementation.cmd,
                        called().with_args(tokens=['cmd', 'key'], interpreter=interpreter))
            assert_that(filter_factory.create_include_filter,
                        called().with_args('regexp', output_stream))
        ...
        def test_executes_command_connected_to_exclude_filter(
            ...
            assert_that(cmds_implementation.cmd,
                        called().with_args(tokens=['cmd', 'key'], interpreter=interpreter))
            assert_that(filter_factory.create_exclude_filter,
                        called().with_args('regexp', output_stream))
